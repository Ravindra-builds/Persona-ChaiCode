...
export function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggleTheme}
      className="relative inline-flex h-10 w-20 items-center rounded-full border border-slate-200 bg-white/80 p-1 shadow-sm transition-all duration-300 hover:bg-white focus:outline-none focus:ring-2 focus:ring-[#d6b36a]/50 focus:ring-offset-2 dark:border-white/10 dark:bg-slate-900/70 dark:hover:bg-slate-800 dark:focus:ring-offset-slate-950"
      aria-label="Toggle theme"
      type="button"
    >
      <span
        className={`absolute left-1 top-1/2 h-8 w-8 -translate-y-1/2 rounded-full bg-slate-950 shadow-md transition-transform duration-300 dark:bg-stone-50 ${isDark ? "translate-x-10" : "translate-x-0"}`}
      />
      ...
      <span className={`relative z-10 ml-auto mr-1 flex h-8 w-8 items-center justify-center ${isDark ? "text-amber-400" : "text-slate-400"}`}>
        <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8">
